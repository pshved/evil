...
# Commit source requests
every 1.minute do
  api_call = %Q(curl #{app_url}/api/commit_sources)
  (1.minute / SOURCE_UPDATE_CACHE_TIME).times {|m| command %Q(sleep #{SOURCE_UPDATE_CACHE_TIME*m}; #{api_call})}
end
