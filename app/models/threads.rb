class Threads < ActiveRecord::Base
  ...
  has_many :faster_posts,
    ...
    :finder_sql => proc { "select posts.id, text_items.body as title, posts.created_at, posts.empty_body, posts.parent_id, posts.marks, posts.unreg_name, users.login as user_login, posts.host, clicks.clicks, hidden_posts_users.action as hide_action,  text_containers.filter as body_filter, text_containers.updated_at as cache_timestamp,
      deleted
    from posts
